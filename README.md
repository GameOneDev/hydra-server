...
- `GET /profile/cloud-saves/all-snapshots` — every committed snapshot of the
  user, each tagged with its game (and cached game name/cover), for the
  launcher's Cloud Save Manager. Each entry also reports how many replaced
  versions the game still holds and what they weigh, so the manager can show
  the whole cost of a save rather than just its current version
...
- **Profiles with nothing here.** An empty `GET /users/{id}/souvenirs` sends
  the launcher to official Hydra for that profile, so a Hydra Cloud
  subscriber's souvenirs still show to your members — whether they have never
  signed in here or joined with a tab already full over there. Which of the two
  it was is reported as `isMember`. A profile this server doesn't know reports
  `hiddenReason: null` rather than "private": claiming a stranger's souvenirs
  are hidden would render a locked tab and stop the fallback.
